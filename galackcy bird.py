...
car = pygame.image.load('poor bird.png')
road = pygame.image.load('galaxcy.png')
coin = pygame.image.load('bitcoin.png')
inc = pygame.image.load('$.png')
...
pygame.display.set_caption("game(www.punk.com)")
...
def gameloop():
	...
	while True:
		...
		text = smallfont.render('punk earned: '+ str(points),True,green)
		...
		if check_step_out(CARX):
			...
			if CARX>0.78*WINDOWWIDTH:
				for i in range(0,20):
					...
					text = smallfont.render('PUNK EARNED: '+ str(points),True,black)
					DISPLAY.blit(text,(0,0))
					clock.tick(FPS)
					pygame.display.update()
			game_over()
